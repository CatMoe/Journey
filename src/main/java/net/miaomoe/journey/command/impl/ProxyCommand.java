...
public class ProxyCommand extends AbstractCommand {
    private final AbstractCommand proxy;
    public ProxyCommand(final @NotNull AbstractCommand proxy) {
        super(Preconditions.checkNotNull(proxy, "proxy").getJourney());
        this.proxy = proxy;
    }

    @Override public void onCommand(@NotNull CommandInvocation invocation) {
        this.proxy.onCommand(invocation);
    }

    @Override @Nullable public List<String> onTabComplete(@NotNull CommandInvocation invocation) {
        return this.proxy.onTabComplete(invocation);
    }
}
